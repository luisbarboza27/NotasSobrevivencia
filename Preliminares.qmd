...
En este caso la función de densidad es:
  $$f(x)=-\frac{dS(x)}{dx}=\lambda \alpha x^{\alpha-1}\exp(-\lambda x^{\alpha})$$
entonces:
$$h(x)= \lambda \alpha x^{\alpha-1}$$
Note que si $\alpha=1$ entonces $h(x)=h\alpha$, si $\alpha>1$ la función de riesgo es creciente y si $\alpha<1$ es decreciente.
:::
...
Si $p(x_j)=\frac 1 3$ para $j=1,2,3$ entonces:
$$S(x)=P(X>x)=
  \begin{cases}
  1 & 0\leq x <1\\
  2/3 & 1\leq x <2 \\
...
Mínimo $x_p$ tal que $S(x_p)\leq 1-p$:
  $$\inf\{t:S(t)\leq 1-p\}$$
En el caso continuo: $S(x_p)=1-p$.
:::
...
$$S(x_{0.5})=e^{-\lambda x_{0.5}}=\frac 1 2 \Rightarrow x_{0.5}=\frac{\log 2}{\lambda}$$
  ...
  Vida media residual (usando la propiedad de pérdida de memoria):
  $$mrl(x)=E[X-x|X>x]=E[X]=\frac 1 \lambda$$

  **Weibull:**:
  $$1-p=\exp(-\lambda x^{\alpha}_p)\Rightarrow x_p=\left[-\frac{\log(1-p)}{\lambda}\right]^{1/\alpha}$$
:::
...
Sea $X\sim Weibull(\lambda, \alpha)$. Si $Y=\log X$ entonces $Y=\mu+\sigma W$ donde $mu=-\frac{\log \lambda}{\alpha}$, $\sigma=\frac 1 \alpha$ y $W\sim \text{VE-Estandard}$, es decir:
$$f_W(w)=\exp\left(w-e^w\right)\qquad w\in \mathbb R$$
...
\end{align*}
donde $S_0$ es la función de sobrevivencia Weibull.
:::
...
::: callout-tip
## Ejemplo: Regresión Weibull
En el caso Weibull: $h_0(x)=\alpha \lambda x^{\alpha-1}$ y la función de riesgo condicional en el caso de un modelo de Cox es:
$$h(x|Z)=\alpha \lambda x^{\alpha-1}\exp(\beta^T Z)$$
...
<!-- ```{definition, label="censura", name="Censura"} -->
...
<!-- ```{definition, label="trunc", name="Truncamiento"} -->
...
<!-- ```{definition, label="censI", name="Censura Tipo I"} -->
...
<!-- ```{example, label="ex7", name="Ejemplo de Censura"} -->
...
<!-- ```{example, label="ex8", name="Caso Exponencial"} -->
